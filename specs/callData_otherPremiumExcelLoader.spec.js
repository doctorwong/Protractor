...
describe('The Call Data - Premium Call Excel Loader', () => {
  ...
  it('should be accessible from the flyout menu', async () => {
    ...
    browser.sleep(1000)
    helperPage.click(flyOutPO.selectOtherPremiumExcelLoader);
    browser.sleep(1000)
    expect(browser.getCurrentUrl()).toEqual('http://' + browser.params.ENV + 'intraapps.ncci.com/rpf/#/other-premium-excel-loader');
  });

  it('should contain a link to download np4 excel loader', async () => {
    browser.get('http://' + browser.params.ENV + 'intraapps.ncci.com/rpf/#/other-premium-excel-loader');
    browser.sleep(1000)
    helperPage.click(callDataPO.downloadOtherPremiumExcelLoader);
  ...
  it('should contain a link to upload np4 excel loader', async () => {
    helperPage.waitUntilClickabilityOf(callDataPO.uploadOtherPremiumExcelLoader);
  })
})